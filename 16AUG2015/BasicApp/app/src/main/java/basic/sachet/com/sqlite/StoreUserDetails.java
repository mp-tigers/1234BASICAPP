...
public class StoreUserDetails extends SQLiteOpenHelper {
    ...
    public void addUser(User user) {
        ...
        for(int i =0; i< institutes.size(); i++)
        {
            ...
            if (checkInstituteInDb(institute.getName(),institute.getCity()) > 0)
                continue;
            values.put("INSTITUTE_NAME", institute.getName().toUpperCase());
            values.put("INSTITUTE_CITY", institute.getCity().toUpperCase());
    ...
    public int checkInstituteInDb(String name, String city)
    {
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursor = db.rawQuery("SELECT * FROM USER_DTLS WHERE INSTITUTE_NAME="+name.toUpperCase()+" AND INSTITUTE_CITY="+city.toUpperCase(),null);
